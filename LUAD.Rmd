...
In order to examine the pattern between the variables containing the missing value by drawing a plot, let's take only the variables with missing values from all the variables
...
The missing patterns can be visualized using VIM package.
...
In the code below we'll drop the index, if there is a case like that if patient vital status is dead but days_to_death is 0.

Because as we mentioned above **There is a direct realtionship between *days_to_last_follow_up* and *days_to_death* columns** 

Also, in *days_to_death* column values are NaN for **alive** patients and there is just information about **dead** patients how long they lived.
