...
Let's applying Oversampling algorithm to data.
```{r}
#We're going to keep the imbalance data as 'imbalance_data' and 'data_new' is going to be our balanced data.
imbalance_data <- data_new

#Apply Oversampling to data_new in order to get balanced data
...
The above plot shows the bend at the eighteenth principal component, it is useful to decide how many PCs to retain for further analysis. In this case we have 20 PCs and we can see that the first 20 PCs explain most of the variability in the data.
...
Now we keep new data with name 'transformed_data' which have fewer columns to create a more powerful model.
```{r}
...
confusionMatrix(test_data$days_to_death,as.factor(fitted.results))
...
```
As you see above model accuracy is 74 %. It is good but let's see if we can increase this ratio with PCA dataset(transformed data) which we created above.
...
confusionMatrix(test_data_pca$days_to_death,as.factor(fitted.results))
...
#Now we can build our linear regression model with our data
#model_missing <- glm(days_to_death ~.,family=binomial,data=train_data_missing)

#Let’s see what it can be able to our model?
#fitted_results <- predict(model_missing,subset(test_data_missing,select=-c(days_to_death)),type='response')
#fitted.results <- ifelse(fitted.results > 0.5,1,0)

#confusionMatrix(test_data_missing$days_to_death,as.factor(fitted.results))
...
- We’re going to seperate the target(days_to_death) before applying k-means.
```{r}
x_data <- subset(data_new, select = -c(days_to_death))
...
table(predicted=hier_cut, true=data_new$days_to_death)
```
## *8*) Apply at least 2 Classification Techniques(Logistic Regression and Random Forest)
...
- *8.1)* Logistic Regression: We have already apply the Logistic Regression in **Step 6** then we can move forward with step 8.2.
...
Our out of bag(OOB) error is 4.93%, so the train data set model accuracy is around 95%.
...
We have a very high accuracy with test data with 91.8%. Now let's plot the top 10 most important features and see the affects of them.
```{r}
varImpPlot(rf,
           sort = T,
           n.var = 10,
...
- Now try to increase the number of trees in the Random Forest and see the changes in the accuracy.
```{r}
rf <- randomForest(days_to_death~., data=train_data, proximity=TRUE, ntree = 1000)
p1 <- predict(rf, test_data)
confusionMatrix(p1, test_data$days_to_death)
```
After increasing the number of trees from 500 to 1000 there is an improvement on the algorithm from *91.8%* to *92.4%*. This means that we can correctly classify the 6 people more among 1000 patients. In real life, it means a lot and we succeeded that by just increasing the number of trees.

- *b)* We have *74.2%* Accuracy in Logistic Regression and *92.4%* with Random Forest. There is a huge difference between these two algorithms. Due to we have a large number of features and Random Forest is a little bit more sophisticated algorithm that could be why we have much better accuracy with Random Forest compared to Logistic Regression.
