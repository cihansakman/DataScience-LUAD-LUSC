...
```{r}
paste("Before Missing value imputation we're going to keep the data with missing values as 'data_new_w_missing_values'")
data_new_w_missing_values <- data_new
...
paste("We're going to keep the imbalance data as 'train_imbalance' and 'test_imbalance'")

train_imbalance <- train
test_imbalance <- test

train <- SMOTE(subset(train, select = -c(days_to_death)),subset(train, select = c(days_to_death)),K=5)
train <- train$data

test <- SMOTE(subset(test, select = -c(days_to_death)),subset(test, select = c(days_to_death)),K=5)
test <- test$data


names(train)[names(train) == 'class'] <- 'days_to_death'
names(test)[names(test) == 'class'] <- 'days_to_death'

p1 <- ggplot(train, aes(x=reorder(days_to_death, days_to_death, function(x)-length(x)))) +
geom_bar(fill='orange') +  labs(x='Classes(0: live < 5 years, 1: live > 5 years)', y="Count") + ggtitle("Train Data")
p2 <- ggplot(test, aes(x=reorder(days_to_death, days_to_death, function(x)-length(x)))) +
geom_bar(fill='orange') +  labs(x='Classes(0: live < 5 years, 1: live > 5 years)', y="Count") + ggtitle("Test Data")
require(gridExtra)
grid.arrange(p1, p2, nrow=2)
```
